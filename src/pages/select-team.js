    ...
    (async () => {
      try {
        ...
        if ((router?.query?.nextRound || "").toString() === "boss") {
          ...
          const isFinalEvolution = (mon) =>
            !namesWithEvos.has(String(mon?.name || "").toLowerCase());
          ...
          const byTypeFinal = pool.filter(
            (m) =>
              isFinalEvolution(m) &&
              (m.types || []).some((t) =>
                allowed.includes(String(t).toLowerCase())
              )
          );
          ...
          if (nr === "3") {
            const gengar = (all || []).find(
              (p) => String(p?.name).toLowerCase() === "gengar"
            );
            if (gengar) picks.push(gengar);
            const poolEx = byTypeFinal.filter(
              (m) => String(m?.name).toLowerCase() !== "gengar"
            );
          ...
          if (picks.length < ENEMIES) {
            ...
            const chosenNames = new Set(
              picks.map((p) => String(p.name).toLowerCase())
            );
            const finalsLeft = finalsAny.filter(
              (m) => !chosenNames.has(String(m.name).toLowerCase())
            );
          ...
          if (picks.length < ENEMIES) {
            const poolLeft = pool.filter(
              (m) => !picks.some((x) => x.id === m.id)
            );
        ...
        try {
          ...
          if (nr === "boss") {
            ...
            const roundIndex = Number.isNaN(n)
              ? 0
              : Math.max(0, Math.min(3, n - 1));
        ...
        <div className={styles.lineup}>
          ...
          <div className={styles.teamSide}>
            <img
              className={`${styles.trainer} ${
                trainerId === 2 ? styles.trainerAdjust : ""
              }`}
          ...
          <div className={styles.vsContainer}>
            ...
            <div className={styles.battleTitle}>
              {nextRound === "boss" ? "BATALHA FINAL!" : `${nextRound}ª BATALHA!`}
            </div>
          </div>

          {/* Lado do inimigo (treinador à esquerda, pokémons à direita) */}
          <div className={styles.teamSide}>
            <img
              ...
              src={
                nextRound === "boss"
                  ? "/images/gary.png"
                  : `/images/enemy${parseInt(nextRound || "1", 10)}.png`
              }
              width={80}
              height={80}
