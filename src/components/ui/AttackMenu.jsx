...
export default function AttackMenu({
  ...
  const flat = [...groups.physical, ...groups.special, ...groups.status, ...groups.unknown];
  ...
  const onMove = (ev) => {
    ...
    if (left + ttWidth > vw + (window.scrollX || 0)) {
      left = Math.max(
        margin,
        Math.round((ev.clientX || 0) + (window.scrollX || 0) - ttWidth - margin),
      );
    }
    if (top + ttHeight > vh + (window.scrollY || 0)) {
      top = Math.max(
        margin,
        Math.round((ev.clientY || 0) + (window.scrollY || 0) - ttHeight - margin),
      );
    }
      ...
      {hover.show && hover.mv && (
        <Tooltip top={hover.pos.top} left={hover.pos.left}>
          <div className={styles.tooltipHeader}>
            ...
            <span className={`${styles.badge} ${styles[hover.mv.type] || ''}`}>
              {tLabel(hover.mv.type || 'normal')}
            </span>
          </div>
          ...
          <div className={styles.tooltipList}>
            {(() => {
              ...
              return defs.map((d, i) => (
                <div key={`st-${i}`}>
                  • {d.target === 'self' ? 'Usuário' : 'Alvo'} {labels[d.stat] || d.stat}{' '}
                  {d.delta > 0 ? '+' : ''}
                  {d.delta}
                </div>
              ));
            })()}
            {Array.isArray(hover.mv.effects) &&
              hover.mv.effects.length > 0 &&
              hover.mv.effects.map((e, i) => (
                <div key={`se-${i}`}>
                  • {String(e.type)} ({e.chance ?? 0}%)
                </div>
              ))}
            {(() => {
              const key = (hover.mv?.name || hover.mv?.id || '').toString().toLowerCase();
